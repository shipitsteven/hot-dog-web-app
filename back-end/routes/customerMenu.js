...
router.post('/order', async (req, res, next) => {
  ...
  // get new user ID
  let getNewUserIDQuery = `SELECT USER_ID FROM USERS
  ...
  let getLatestLogEventQuery = `SELECT LOG_ITEM_ID FROM LOG
    ORDER BY LOG_ITEM_ID DESC
    LIMIT 1
    ;`;
  let latestLogID = (await db.promise().query(getLatestLogEventQuery))[0][0]
    .LOG_ITEM_ID;

  // insert log event into user logs table
  let userLogsQuery = `INSERT INTO USER_LOGS(USER_ID, LOG_ITEM_ID)
  ...
  let latestID = (await db.promise().query(getLatestOrderIDQuery))[0][0]
    .ORDER_ID;
